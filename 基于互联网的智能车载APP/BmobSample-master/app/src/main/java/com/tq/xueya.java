...
public class xueya extends Activity {
    //	曲线
...
//			addY=250;
            ...
            if(gx<200){
                if(hua[20]>1){
                    Toast.makeText(xueya.this, "请用您的指尖盖住摄像头镜头！", Toast.LENGTH_SHORT).show();
                    hua[20]=0;}
                hua[20]++;
                return;}
            else
...
//获取系统结束时间（ms）
            ...
            if (totalTimeInSecs >= 2) {
                ...
                int gao=beatsAvg+40;
                int di=beatsAvg+5;
                text.setText("高压："+String.valueOf(gao)+"  底压:"+String.valueOf(di));
//获取系统时间（ms）
                startTime = System.currentTimeMillis();
                beats = 0;
            }
